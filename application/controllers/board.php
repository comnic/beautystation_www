...
class Board extends MH_Controller {
	...
	public function index()
	{
		...
		$this->load->view('board/board_list_gallery', array("data"=>$data));
	}
	
	public function read($idx){		
		$this->_head();
		$data = $this->board_model->getContent($idx);
		$this->load->view('board/board_view', array("data"=>$data));
	...
	public function write($bid){
		...
		if ($this->form_validation->run() == FALSE)
		{
			if(count($this->_segment) >= 4 && $this->_segment[4] == "reply")
				$this->load->view('board/board_write', array("bid"=>$bid, "is_reply"=>"Y", "parent"=>$this->_segment[5]));
			else
				$this->load->view('board/board_write', array("bid"=>$bid, "is_reply"=>"N", "parent"=>"0"));
		}
		else
		{
	...
	function _head(){
		$this->load->view("head");
		$this->load->view("board/board_head");
	}
